...
impl TypeCheck for Value {
    fn type_check(&mut self, gamma: TypeEnviroment, _lifetime: usize) -> Result<(TypeEnviroment, Type), String> {
        match self {
        ...
        match self {
                ...
                match gamma.get(name)?.value {
                    Type::Function { args, ret } => {
                        if args.len() != params.len() {
                            return Err(TypeError::FunctionCallIncompatableArgumentCount(args.len(), params.len()).to_string())
                        };
                        let mut g1 = gamma;
                        for (arg, param) in args.iter().zip(params.iter_mut()) {
                            ...
                            if !shape_compatible(&g2, &arg.clone(), &t.clone()) {
                                return Err(TypeError::FunctionCallIncompatableArgumentType(arg.clone(), t.clone()).to_string())
                            };
                            g1 = g2;
                        }
            ...
            Term::FunctionDeclaration { name: fn_name, args, body, ty } => {
                ...
                match ty {
                    Some(ty) => {
                        if t != *ty {
                            return Err(TypeError::FunctionUnexpectedReturn(t.clone(), ty.clone()).to_string())
                        }
                    },
                    None => {}
                }
            ...
            Term::Ref { mutable, var } => {
                if *mutable {
                    if write_prohibited(&gamma, var.clone()) {
                        return Err(TypeError::MutrefAlreadyBorrowedImmut(var.get_name()).to_string())
                    };
            ...
            Term::Let { variable, term, .. } => {
                ...
                match t {
                    Type::Epsilon => return Err(TypeError::LetExprNoReturn(*term.clone()).to_string()),
                    _ => {}
                }
